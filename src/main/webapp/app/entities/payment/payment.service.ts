...
export class PaymentService {
  ...
  protected convertDateFromClient(payment: IPayment): IPayment {
    const copy: IPayment = Object.assign({}, payment, {
      timeStamp: payment.timeStamp && payment.timeStamp.isValid() ? payment.timeStamp.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((payment: IPayment) => {
        payment.timeStamp = payment.timeStamp ? moment(payment.timeStamp) : undefined;
      });
    }
    return res;
  }
