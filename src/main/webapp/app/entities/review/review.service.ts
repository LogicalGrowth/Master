...
export class ReviewService {
  ...
  protected convertDateFromClient(review: IReview): IReview {
    const copy: IReview = Object.assign({}, review, {
      timeStamp: review.timeStamp && review.timeStamp.isValid() ? review.timeStamp.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((review: IReview) => {
        review.timeStamp = review.timeStamp ? moment(review.timeStamp) : undefined;
      });
    }
    return res;
  }
